...
function Legend() {
    return (
        <div className='text-center'>
          {categoryList.map((cat: string, idx: number) => <span key={idx} role="button" data-tip={categoryToDescriptionEnum[cat as keyof typeof categoryToDescriptionEnum]} className={`badge rounded-pill me-4 mb-2 bg-${categoryToColorEnum[cat as keyof typeof categoryToColorEnum]} ${categoryToColorEnum[cat as keyof typeof categoryToColorEnum] === 'light' ? "text-dark" : ""}`} >{cat}</span>)}
